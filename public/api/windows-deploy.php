...
$lines[] = 'VIRTIO_FOUND=""';
$lines[] = '# Search for VirtIO ISO on configured storage';
$lines[] = 'echo "    DEBUG: pvesm list $ISO_STOR --content iso:"';
$lines[] = 'pvesm list "$ISO_STOR" --content iso 2>&1 || true';
$lines[] = 'echo "    DEBUG: ls -la $ISO_STOR_PATH/virtio*:"';
$lines[] = 'ls -la "$ISO_STOR_PATH"/virtio* 2>&1 || true';
$lines[] = 'echo "    DEBUG: file $ISO_STOR_PATH/virtio-win.iso:"';
$lines[] = 'file "$ISO_STOR_PATH/virtio-win.iso" 2>&1 || true';
$lines[] = '';
...
$lines[] = 'else';
$lines[] = '  # Try finding any virtio ISO name via pvesm';
...
$lines[] = '    if qm set $VMID --ide0 "$VIRTIO_VOL",media=cdrom 2>/dev/null; then';
...
$lines[] = 'fi';
$lines[] = '';
$lines[] = '# If still not found, check if file exists but is broken (0 bytes) and re-download';
$lines[] = 'if [ -z "$VIRTIO_FOUND" ]; then';
$lines[] = '  if [ -f "$ISO_STOR_PATH/virtio-win.iso" ] && [ ! -s "$ISO_STOR_PATH/virtio-win.iso" ]; then';
$lines[] = '    echo "    VirtIO ISO exists but is 0 bytes — removing broken file..."';
