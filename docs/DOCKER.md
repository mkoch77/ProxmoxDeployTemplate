# ProxmoxDeploy – Docker-Installation (Ubuntu 24.04)
...
mkdir -p /opt/proxmoxdeploy
cd /opt/proxmoxdeploy

# Repository klonen
git clone https://github.com/your-org/ProxmoxDeployTemplate.git .
```

---

## 3. SSH-Schlüssel vorbereiten
...
> Für Let's Encrypt zusätzlich `DOMAIN` und `LETSENCRYPT_EMAIL` setzen – siehe Abschnitt 6.

---

## 5. TLS-Zertifikat und Start
...
```bash
mkdir -p /opt/proxmoxdeploy/docker/nginx/certs

# Zertifikat und Schlüssel an die richtige Stelle kopieren
cp /etc/ssl/certs/mein-zertifikat.crt /opt/proxmoxdeploy/docker/nginx/certs/cert.pem
cp /etc/ssl/private/mein-schluessel.key /opt/proxmoxdeploy/docker/nginx/certs/key.pem
chmod 644 /opt/proxmoxdeploy/docker/nginx/certs/cert.pem
chmod 600 /opt/proxmoxdeploy/docker/nginx/certs/key.pem
...
```bash
bash /opt/proxmoxdeploy/docker/letsencrypt-init.sh
```

Das Skript:
- Lässt Certbot das Zertifikat per HTTP-01-Challenge ausstellen
- Kopiert die Zertifikate nach `/opt/proxmoxdeploy/docker/nginx/certs/`
...
mkdir -p /var/log/proxmoxdeploy

# Cron-Job einrichten
echo "0 3 * * * root bash /opt/proxmoxdeploy/docker/letsencrypt-renew.sh >> /var/log/proxmoxdeploy/letsencrypt.log 2>&1" \
  > /etc/cron.d/proxmoxdeploy-letsencrypt

chmod 644 /etc/cron.d/proxmoxdeploy-letsencrypt
```

---

## 6. DRS Cron-Job (automatisches Load-Balancing)
...
echo "*/5 * * * * root docker compose -f /opt/proxmoxdeploy/docker-compose.yml exec -T app php /var/www/html/cli/drs-run.php >> /var/log/proxmoxdeploy/drs.log 2>&1" \
  > /etc/cron.d/proxmoxdeploy-drs

chmod 644 /etc/cron.d/proxmoxdeploy-drs
```
...
docker run --rm \
  -v proxmoxdeploy_data:/data \
  -v /var/backups/proxmoxdeploy:/backup \
  alpine tar czf /backup/db-$(date +%Y%m%d-%H%M).tar.gz -C /data .

# Backup-Verzeichnis anlegen
mkdir -p /var/backups/proxmoxdeploy

# Automatisches tägliches Backup per Cron
echo "0 2 * * * root docker run --rm -v proxmoxdeploy_data:/data -v /var/backups/proxmoxdeploy:/backup alpine tar czf /backup/db-\$(date +\%Y\%m\%d).tar.gz -C /data . && find /var/backups/proxmoxdeploy -name '*.tar.gz' -mtime +30 -delete" \
  > /etc/cron.d/proxmoxdeploy-backup

chmod 644 /etc/cron.d/proxmoxdeploy-backup
```
...
```bash
# App-Logs
docker compose -f /opt/proxmoxdeploy/docker-compose.yml logs -f app

# Nginx-Logs
docker compose -f /opt/proxmoxdeploy/docker-compose.yml logs -f nginx

# Alle Container
docker compose -f /opt/proxmoxdeploy/docker-compose.yml logs -f
```
...
docker compose -f /opt/proxmoxdeploy/docker-compose.yml exec app ls -la /ssh/

# SSH-Verbindung direkt aus dem Container testen
docker compose -f /opt/proxmoxdeploy/docker-compose.yml exec app \
...
docker compose -f /opt/proxmoxdeploy/docker-compose.yml exec app ls -la /var/www/html/data/
...
```bash
docker compose -f /opt/proxmoxdeploy/docker-compose.yml logs app --tail=50
```
